...
local function pack(plugins)
	...
	for _, p in ipairs(plugins) do
		if p.config then
			...
			if not ok then
				vim.notify(("plugin '%s' config failed: %s"):format(p.src, err), vim.log.levels.ERROR)
			end
		end
	end
end
...
function M.init(keybinds, symbols)
	...
	pack({
			...
			config = function()
				...
				for _, toggle in ipairs(keybinds.snacks.toggles) do
					if toggle.option then
						Snacks.toggle.option(toggle.option, { name = toggle.name }):map(toggle.key)
		...
		--------------------------------------------------------------------
		{
			...
			config = function()
				...
				local sc_diff_integration = function()
					local ns_id = vim.api.nvim_create_namespace("sourcecontrol_diff")
					...
					return function()
						...
						if not vim.b[buf].sc_diff_active then
							return {}
						end
						...
						for _, mark in ipairs(marks) do
							...
							if details.line_hl_group == "DiffAdd" then
								table.insert(out, { line = line, hl_group = add_hl })
							elseif details.line_hl_group == "DiffDelete" then
								table.insert(out, { line = line, hl_group = del_hl })
							end
		...
		-- 			vim.api.nvim_buf_set_mark(0, ">", erow + 1, math.max(0, ecol - 1), {})
		-- 			vim.cmd("normal! gv")
		-- 		end
		--
