...
if vim.fn.filereadable(editorconfig_file) == 1 then
	...
	vim.notify("EditorConfig enabled from " .. editorconfig_file,
		vim.log.levels.INFO)
end
...
function M.init(keybinds, symbols)
	...
	local signs = {
		{ name = "DiagnosticSignError", text = symbols.ui.error },
		{ name = "DiagnosticSignWarn",  text = symbols.ui.error },
		{ name = "DiagnosticSignHint",  text = symbols.ui.hint },
		{ name = "DiagnosticSignInfo",  text = symbols.ui.info },
	}
	...
	do
		...
		local accept_key = ok and type(inline.get_accept_key) == "function" and
			inline.get_accept_key() or nil
	...
	vim.api.nvim_create_user_command("LspDebug", function()
		...
		for _, client in pairs(clients) do
			...
			vim.notify(" Supports definition: ",
				client.server_capabilities.definitionProvider or false)
			vim.notify(" Supports declaration: ",
				client.server_capabilities.declarationProvider or false)
			vim.notify(" Supports references: ",
				client.server_capabilities.referencesProvider or false)
	...
	for _, module in ipairs(vim.fn.readdir(config_path)) do
		...
		if name and name ~= "init" and name ~= "utils" then
			...
			if not ok then
				vim.notify("Failed to load LSP module: " .. name,
					vim.log.levels.WARN)
			end
		end
	end
